...
/// Trung gian gom tất cả “cờ khóa” input ở Player (hit/parry/dead…)
public sealed class PlayerInputGate : MonoBehaviour
{
    ...
    readonly HashSet<object> _parryPoseSources = new(); // nhiều nguồn cùng khóa parry pose
    float _parrySuccUntilUnscaled;                     // i-frame sau parry OK (unscaled)
    bool _deadLocked;                                  // khóa toàn phần khi chết
    ...
    public void SetDeadLocked(bool v)
    {
        _deadLocked = v;
    ...
    public bool IsDeadLocked => _deadLocked;

    // Quy tắc cho phép/không cho phép
    public bool CanAttack => !_deadLocked && !_hitLocked && !IsParryPoseLocked;
    public bool CanParry => !_deadLocked && !_hitLocked && !IsParryPoseLocked;
}
